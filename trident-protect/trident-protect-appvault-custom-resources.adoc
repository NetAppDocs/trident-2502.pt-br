...
É necessário criar um AppVault CR manualmente ou usando a linha de comando ao executar operações de proteção de dados em um aplicativo, e o AppVault CR precisa residir no cluster onde o Trident Protect está instalado.  O pedido de configuração (CR) do AppVault é específico para o seu ambiente; você pode usar os exemplos desta página como guia ao criar pedidos de configuração do AppVault.
...
Ao criar objetos AppVault usando CRs ou o plugin Trident Protect CLI, você pode, opcionalmente, instruir o Trident Protect a usar um segredo do Kubernetes que contenha senhas personalizadas para a criptografia dos repositórios Restic e Kopia. Se você não especificar uma senha secreta, o Trident Protect usará uma senha padrão.
