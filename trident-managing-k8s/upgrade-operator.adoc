...
NOTE: Para atualizar a partir do Trident instalado usando o operador com escopo de namespace (versões 20.07 a 20.10), use as instruções de atualização para a sua versão instalada do Trident.
...
.Passos
. Se você link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-trident-using-helm["Trident instalado usando Helm"], pode usar  `helm upgrade trident netapp-trident/trident-operator --version 100.2502.0` para atualizar em uma única etapa. Se você não adicionou o repositório Helm ou não consegue usá-lo para atualizar:
+
...
NOTE: Antes de alternar entre os métodos de instalação, revise link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["Movendo-se entre os métodos de instalação"].
